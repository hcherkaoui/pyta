...
d = datetime.now()
root_outdir = 'results_prox_t_{0}{1}{2}{3}{4}'.format(d.year, d.month, d.day,
                                                      d.hour, d.minute)
...
input_snr = 1.0
random_state = 42
params = dict(tr=tr, nx=nx, ny=ny, nz=nz, N=N, h=h,
...
params = dict(t_r=tr, len_h=len_h, n_jobs=-2, lbda_type='ratio', lbda=1.0e-2,
              solver_kwargs=dict(max_iter=250), unbiased=True, verbose=1)
print("-"*80)
...
# Signal recovery
print("-"*80)
print("Running the temporal proximal operator analysis...\n")
t0 = time.time()
